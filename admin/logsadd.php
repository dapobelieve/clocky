...
class clogs_add extends clogs {
	...
	function LoadDefaultValues() {
		...
		$this->out->OldValue = $this->out->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->in->FldIsDetailKey) {
			$this->in->setFormValue($objForm->GetValue("x_in"));
			$this->in->CurrentValue = ew_UnFormatDateTime($this->in->CurrentValue, 5);
		}
		if (!$this->out->FldIsDetailKey) {
			$this->out->setFormValue($objForm->GetValue("x_out"));
			$this->out->CurrentValue = ew_UnFormatDateTime($this->out->CurrentValue, 5);
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->item->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->item->FldCaption()));

			// in
			$this->in->EditCustomAttributes = "";
			$this->in->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->in->CurrentValue, 5));
			$this->in->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->in->FldCaption()));

			// out
			$this->out->EditCustomAttributes = "";
			$this->out->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->out->CurrentValue, 5));
			$this->out->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->out->FldCaption()));
	...
	function ValidateForm() {
		...
		if (!ew_CheckDate($this->in->FormValue)) {
			ew_AddMessage($gsFormError, $this->in->FldErrMsg());
		}
		if (!ew_CheckDate($this->out->FormValue)) {
			ew_AddMessage($gsFormError, $this->out->FldErrMsg());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// in
		$this->in->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->in->CurrentValue, 5), NULL, FALSE);

		// out
		$this->out->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->out->CurrentValue, 5), NULL, FALSE);
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "logslist.php", $this->TableVar);
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($logs->in->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_out");
			if (elm && !ew_CheckDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($logs->out->FldErrMsg()) ?>");
...
<input type="text" data-field="x_name" name="x_name" id="x_name" size="30" maxlength="200" placeholder="<?php echo $logs->name->PlaceHolder ?>" value="<?php echo $logs->name->EditValue ?>"<?php echo $logs->name->EditAttributes() ?>>
</span>
<?php echo $logs->name->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($logs->item->Visible) { // item ?>
	<tr id="r_item">
		<td><span id="elh_logs_item"><?php echo $logs->item->FldCaption() ?></span></td>
		<td<?php echo $logs->item->CellAttributes() ?>>
<span id="el_logs_item" class="control-group">
<textarea data-field="x_item" name="x_item" id="x_item" cols="35" rows="4" placeholder="<?php echo $logs->item->PlaceHolder ?>"<?php echo $logs->item->EditAttributes() ?>><?php echo $logs->item->EditValue ?></textarea>
</span>
<?php echo $logs->item->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($logs->in->Visible) { // in ?>
	<tr id="r_in">
		<td><span id="elh_logs_in"><?php echo $logs->in->FldCaption() ?></span></td>
		<td<?php echo $logs->in->CellAttributes() ?>>
<span id="el_logs_in" class="control-group">
<input type="text" data-field="x_in" name="x_in" id="x_in" placeholder="<?php echo $logs->in->PlaceHolder ?>" value="<?php echo $logs->in->EditValue ?>"<?php echo $logs->in->EditAttributes() ?>>
</span>
<?php echo $logs->in->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($logs->out->Visible) { // out ?>
	<tr id="r_out">
		<td><span id="elh_logs_out"><?php echo $logs->out->FldCaption() ?></span></td>
		<td<?php echo $logs->out->CellAttributes() ?>>
<span id="el_logs_out" class="control-group">
<input type="text" data-field="x_out" name="x_out" id="x_out" placeholder="<?php echo $logs->out->PlaceHolder ?>" value="<?php echo $logs->out->EditValue ?>"<?php echo $logs->out->EditAttributes() ?>>
</span>
<?php echo $logs->out->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
